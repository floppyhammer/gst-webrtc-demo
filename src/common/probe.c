...
GstPadProbeReturn buffer_probe_cb(GstPad* pad, GstPadProbeInfo* info, gpointer user_data) {
    if (info->type & GST_PAD_PROBE_TYPE_BUFFER) {
        ...
        if (previous_pts != 0) {
            ...
            if (pts_diff > 50) {
                ALOGE("Buffer probe: PTS: %" GST_TIME_FORMAT ", PTS diff: %ld",
                      GST_TIME_ARGS(pts),
                      pts_diff);
            } else {
                ALOGD("Buffer probe: PTS: %" GST_TIME_FORMAT ", PTS diff: %ld",
                      GST_TIME_ARGS(pts),
                      pts_diff);
            }
        }
        ...
        if (gst_buffer_map(buf, &map, GST_MAP_READ)) {
            if (map.size >= 12) {
                ...
                uint16_t seq_num = (data[2] << 8) | data[3]; // For big-endian systems
