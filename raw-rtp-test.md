...
```bash
gst-launch-1.0 -v filesrc location=test.mp4 ! decodebin3 ! x264enc tune=zerolatency bitrate=2000 ! rtph264pay config-interval=-1 aggregate-mode=zero-latency ! application/x-rtp,encoding-name=H264,clock-rate=90000,media=video,payload=96 ! queue ! rtpsink uri=rtp://10.11.9.192:5000
...
gst-launch-1.0 \
  ...
  filesrc location=test.mp4 ! decodebin3 ! timeoverlay ! x264enc key-int-max=60 tune=zerolatency bitrate=2000 ! \
