...
gst-launch-1.0 -v \
  ...
  application/x-rtp,encoding-name=H264,clock-rate=90000,media=video,payload=96 ! udpsink host=10.11.9.192 port=5000
```
...
gst-launch-1.0 -v \
  ...
  rtpjitterbuffer ! rtph264depay ! decodebin3 ! autovideosink sync=false
```
...
```bash
gst-launch-1.0 -v \
  filesrc location=test.mp4 ! decodebin3 ! timeoverlay ! tee name=t1 ! queue ! videoconvert ! \
  autovideosink t1. ! queue ! encodebin2 profile="video/x-h264|element-properties,tune=zerolatency,bitrate=8192" ! \
  rtph264pay config-interval=-1 aggregate-mode=zero-latency ! \
  application/x-rtp,encoding-name=H264,clock-rate=90000,media=video,payload=96 ! \
  rtpulpfecenc percentage=20 pt=122 ! udpsink host=10.11.9.192 port=5000
```
...
gst-launch-1.0 rtpbin latency=500 fec-decoders='fec,0="rtpst2022-1-fecdec\ size-time\=1000000000";' \
  name=rtp udpsrc port=5002 caps="application/x-rtp, payload=96" ! queue ! rtp.recv_fec_sink_0_0 udpsrc port=5004 \
  caps="application/x-rtp, payload=96" ! queue ! rtp.recv_fec_sink_0_1 \
  udpsrc port=5000 caps="application/x-rtp, media=video, clock-rate=90000, encoding-name=mp2t, payload=33" ! \
  queue ! netsim drop-probability=0.0 ! rtp.recv_rtp_sink_0 \
  rtp. ! decodebin ! videoconvert ! queue ! autovideosink sync=false
```
